...
select 
	productline, 
	round(cast(sum(sales) as numeric),2) as revenue 
...
select 
	year_id, 
	count(distinct month_id) as months_operated 
...
with cte as(
	select 
		*, row_number() over(partition by year_id order by revenue desc) as rnk 
	from(
		select 
			year_id, month_id, 
			round(cast(sum(sales) as numeric),2) as revenue, count(ordernumber) as total_orders 
		from sales group by 1,2) as a)
...
with cte as(
	...
	from(
		select 
			year_id, month_id, productline, 
			round(cast(sum(sales) as numeric),2) as revenue, count(ordernumber) as total_orders 
		from sales group by 1,2,3) as a)
...
select 
	customername, rfm_recency, rfm_frequency, rfm_monetary, rfm_cell,
	case
